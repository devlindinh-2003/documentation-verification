...
export default function SellerPage() {
  ...
  const { data: verifications, isLoading } = useQuery<VerificationRecord[]>({
    ...
    refetchInterval: (query) => {
      const hasActive = query.state.data?.some((v) =>
  ...
  const activeCount =
    verifications?.filter((v) => ["pending", "processing"].includes(v.status))
      .length || 0;
  const MAX_PENDING = 5;
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed">
            To ensure a safe marketplace, we require all sellers to verify their
            identity. Upload a valid government-issued ID to get started.
          </p>
        </div>
                ...
                className={`relative overflow-hidden rounded-3xl border-2 transition-all duration-500 p-8 sm:p-10 ${
                  STATUS_CONFIG[latestVerification.status].theme === "green"
                    ...
                    : STATUS_CONFIG[latestVerification.status].theme === "red"
                      ...
                      : STATUS_CONFIG[latestVerification.status].theme ===
                          "yellow"
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-4 flex-1 min-w-0">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-colors shrink-0 ${
              ...
              <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-20 text-center space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-500 max-w-xs mx-auto">
                    Upload your first document to verify your identity and start
                    selling.
                  </p>
                </div>
              </div>
